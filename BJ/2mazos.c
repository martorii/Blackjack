...
double checkGame(int,int,int,int *,int *);
...
int main(void) {
  ...
  int ***mazo,noasesnosplit[17][10],asesnosplit[8][10],split[10][10],p2=0;
  int accio,*ganan;
  double bote;
...
ganan=(int *) malloc(30*sizeof(int));
 for(i=-15;i<16;i++) {
    ganan[i+15]=0;
  }
...
while(p<N) {
 ...
 /*Ara es prepara el compte, que cada 25 partides s'inicialitza a 0*/
  ...
  if(trampa==4) {
    ...
    if(p>0) {
    bote+=checkGame(dealer,auxsplit1,bet1,&cuenta,ganan);
    
     /*Aposta2*/
    
     bote+=checkGame(dealer,auxsplit2,bet2,&cuenta,ganan);
     escriure(p2,bote,sortida);
      p2++;
    }
     p++;
  ...
  bote+=checkGame(dealer,player,bet,&cuenta,ganan);
  escriure(p2,bote,sortida);
  p2++;
  }
  p++;
...
for(i=-15;i<16;i++) {
  escriure(i,ganan[i+15],sortida2);
}
return 0;
}
...
double checkGame(int dealer,int jugador,int bet,int *cuenta, int *ganan) {
  ...
  if(*cuenta>-16&&*cuenta<16) {
  ganan[*cuenta+15]+=bote;
  }
  return bote;
}